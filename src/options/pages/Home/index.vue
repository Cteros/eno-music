    ...
    <div v-if="mainSong" class="w-full grid grid-cols-2 gap-4 px-8">
      ...
      <div class="w-full h-full overflow-auto relative">
        <div class="flex gap-3 flex-col absolute top-0 left-0">
          <div v-for="song in store.musicRankList" :key="song.id" class="flex group transition-all duration-120 cursor-pointer rounded-3 p-2 hover:bg-$eno-fill-1" @click="store.startPlay(song)">
